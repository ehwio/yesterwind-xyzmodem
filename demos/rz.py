...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Connects to an `sz --tcp-server` instance and receives files via ZModem.
Functionally equivalent to `rz --tcp-client <host>:<port>`.
...
Example:
    # On the server:
    sz --tcp-server file.bin
    # (prints something like:  ready at port 12345)

    # On the client:
    uv run demos/rz.py server.host 12345
"""
...
async def receive(host: str, port: int, download_dir: pathlib.Path) -> int:
    ...
    console.print(
        f"[dim]Connected.  Downloads → [cyan]{download_dir}[/cyan][/dim]\n"
    )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("host", help="Hostname or IP of the sz --tcp-server")
    p.add_argument("port", type=int, help="TCP port printed by sz --tcp-server")
    p.add_argument(
        "-d", "--download-dir",
        default="downloads",
        metavar="DIR",
