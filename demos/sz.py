...
Usage:
    uv run demos/sz.py [options] <file> [<file> ...]

Examples:
    # Single shot — send file.bin to whoever connects, then exit:
    uv run demos/sz.py file.bin

    # Keep listening; serve every client that connects (in parallel):
    uv run demos/sz.py --serve-forever file1.bin file2.bin

    # Pick a specific port (default: OS-assigned):
    uv run demos/sz.py --port 12345 archive.tar.gz
...
async def serve(
    ...
    console.print(
        ...
        f"[dim]           or:  uv run demos/rz.py {host} {actual_port}[/dim]\n"
    )
