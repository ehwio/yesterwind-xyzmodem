...
Usage:
    yw-bbs <host> [port] [-d download_dir]

Examples:
    yw-bbs bbs.fozztexx.com
    yw-bbs bbs.example.com 23 -d ~/Downloads/bbs
...
class BBSClient:
    ...
    def __init__(self, host: str, port: int, download_dir: str) -> None:
        ...
        self._neg: _Negotiator | None = None

        self._running = False
        self._in_transfer = False  # gates stdin-to-remote forwarding
        self._orig_tc: list | None = None
...
def main() -> None:
    if not _HAS_RICH:
        print(
            "error: yw-bbs requires the 'rich' package.\n"
